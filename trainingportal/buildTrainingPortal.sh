...
zip ../build/trainingportal/SCD-"$(date)".DOCKER.zip -r *.js *.sql \
 package-lock.json package.json public/* static/* challengeSecrets.json \
 -x "*.DS_Store" -x "encryptConfigs.js" -x "*.config.js" -x "config.js";
rm -f *.zip 
zip SCD-"$(date)".AWS.zip -r *.js *.sql \
 .ebextensions/* \
 node_modules/* public/* static/* challengeSecrets.json \
